...
cworld = {
   ...
   {"[试炼]火的试炼", 'Dawn_TrialsFire'},
    ...
    {"[霞谷]Sunset", 'Sunset'},
    {"[霞谷]Sunset_Citadel", 'Sunset_Citadel'},
    {"[霞谷]Sunset_FlyRace", 'Sunset_FlyRace'},
    {"[霞谷]SunsetRace", 'SunsetRace'},
    {"[霞谷]SunsetEnd", 'SunsetEnd'},
    {"[霞谷]Sunset_YetiPark", 'Sunset_YetiPark'},
    {"[霞谷]SunsetVillage", 'SunsetVillage'},
    {"[霞谷]SunsetColosseum", 'SunsetColosseum'},
    {"[霞谷]SunsetEnd2", 'SunsetEnd2'},
    {"[墓土]DuskStart", 'DuskStart'},
    {"[墓土]Dusk", 'Dusk'},
    {"[墓土]DuskGraveyard", 'DuskGraveyard'},
    {"[墓土]DuskOasis", 'DuskOasis'},
    {"[墓土]Dusk_CrabField", 'Dusk_CrabField'},
    {"[墓土]DuskMid", 'DuskMid'},
    {"[墓土]DuskEnd", 'DuskEnd'},
    {"[禁阁]Night", 'Night'},
    {"[禁阁]Night2", 'Night2'},
    {"[禁阁]NightArchive", 'NightArchive'},
    {"[新季节]NightDesert", 'NightDesert'},
    {"[新季节]Planet", 'NightDesert_Planets'},
    {"[新季节]Night Beach", 'NightDesert_Beach'},
    {"[新季节]Jar cave", 'Night_JarCave'},
    {"办公室", 'TGCOffice'},
    {"伊甸园1", 'StormStart'},
    {"伊甸园2", 'Storm'},
...
function magicmenu()
  gf = gg.choice({'选择魔法（回闪光）','选择魔法（不会闪光）','替换背包中的魔法','无限法术（暂时）', '随机魔法','移除所有法术'},nil,'')
...
function domenu()
      ...
      if m == 15 then
        x=gg.choice({'搜索 1D','打印偏移量','打印动作','打印项目','打印魔术','打印 daily','frags','拿螃蟹','扔螃蟹','撞冥龙','执行','加载坐标'
        },nil,'⚠️这种功能是不稳定的')
        if x == 1 then
